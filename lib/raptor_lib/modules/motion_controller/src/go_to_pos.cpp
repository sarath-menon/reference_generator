...
bool MotionCtrl::go_to_pos(const cpp_msg::Position &current_pos,
  ...
  while (t_counter < max_time) {
    ...
    bool x_reach = check_reached(current_pos.x, target_pos.x, pos_thresholds.x);
    bool y_reach = check_reached(current_pos.y, target_pos.y, pos_thresholds.y);
    bool z_reach = check_reached(current_pos.z, target_pos.z, pos_thresholds.z);

    // std::cout << "Reached flags: " << x_reach_flag << '\t' << y_reach_flag
    //           << '\t' << z_reach_flag << std::endl;

    // exit if position has been reached
    if (x_reach && y_reach && z_reach == true) {
